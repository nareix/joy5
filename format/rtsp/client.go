...
var ErrCodecChange = fmt.Errorf("rtsp: codec data change, please call HandleCodecChange()")
...
func (self *Client) Describe() (err error) {
	var res Response
	...
	for _, media := range medias {
		...
		if err := stream.makeCodec(); err != nil {
			return err
		}
...
func (self *Client) HandleCodecChange() (_newcli *Client, err error) {
	newcli := &Client{}
	*newcli = *self
	...
	for _, stream := range self.streams {
		...
		*newstream = *stream
		...
		if newstream.isCodecChange() {
			newstream.clearCodecChange()
		}
...
func (self *Stream) clearCodecChange() {
	self.spsChanged = false
	self.ppsChanged = false
}

func (self *Stream) isCodecChange() bool {
...
func (self *Stream) handleBuggyAnnexbH264Packet(timestamp uint32, packet []byte) (isBuggy bool, err error) {
	if len(packet) >= 4 && packet[0] == 0 && packet[1] == 0 && packet[2] == 0 && packet[3] == 1 {
		isBuggy = true
		if nalus, typ := h264.SplitNALUs(packet); typ != h264.NALU_RAW {
...
func (self *Stream) makeCodec() (err error) {
	media := self.Sdp
	...
	if media.PayloadType >= 96 && media.PayloadType <= 127 {
		...
		case av.H264:
			...
			if len(self.sps) == 0 || len(self.pps) == 0 {
				if nalus, typ := h264.SplitNALUs(media.Config); typ != h264.NALU_RAW {
					for _, nalu := range nalus {
						if len(nalu) > 0 {
							self.handleH264Payload(0, nalu)
						}
			...
			if self.Codec == nil {
				self.Codec = h264.NewCodec()
			}

			if len(self.pps) > 0 {
				self.Codec.(*h264.Codec).AddSPSPPS(self.pps)
			}
			if len(self.sps) > 0 {
				self.Codec.(*h264.Codec).AddSPSPPS(self.sps)
				self.Codec.(*h264.Codec).ToConfig()
			}

		case av.AAC:
			if self.Codec, err = aac.FromMPEG4AudioConfigBytes(media.Config); err != nil {
				err = fmt.Errorf("rtsp: aac sdp config invalid: %s", err)
				return
			}
		}
	} else {
		err = fmt.Errorf("rtsp: unsoporerted codec type")
	}
...
func (self *Client) Play() (err error) {
	...
	if self.stage == stageSetupDone {
		self.stage = stagePlayDone
	}
	...
	if stream.gotpkt {
		...
		*/
		...
		if pkt.Time < stream.lasttime || pkt.Time-stream.lasttime > time.Minute*30 {
			err = fmt.Errorf("rtp: time invalid  time=%v lasttime=%v", pkt.Time, stream.lasttime)
			return
		}
		stream.lasttime = pkt.Time

		if self.DebugRtp {
			fmt.Println("rtp: pktout", pkt.Time, len(pkt.Data))
		}
...
func (self *Client) ReadPacket() (pkt av.Packet, err error) {
	if err = self.prepare(stagePlayDone); err != nil {
		return
	}
	return self.readPacket()
}

func (self *Client) Streams() ([]interface{}, error) {
	if err := self.prepare(stageDescribeDone); err != nil {
		return nil, err
	}
	var streams []interface{}
	for _, s := range self.streams {
		streams = append(streams, s.Codec)
	}
	return streams, nil
}
