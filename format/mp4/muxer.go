...
func NewMuxer(w av.StreamsWriteSeeker) (*Muxer, error) {
	m := &Muxer{
		w:       w,
		bufw:    bufio.NewWriterSize(w, 256),
		streams: make(map[int]*Stream),
	}
	streams, err := w.Streams()
	if err != nil {
		return nil, err
	}

	if err := m.addStreams(streams); err != nil {
		return nil, err
	}
...
func (self *Muxer) addStreams(codecs []interface{}) (err error) {
	for _, codec := range codecs {
		cType, err := CodecType(codec)
		if err != nil {
			return err
		}
...
func (self *Stream) fillTrackAtom() (err error) {
	...
	switch codec := self.Codec.(type) {
	case *h264.Codec:
		width, height := codec.W, codec.H
		self.sample.SampleDesc.AVC1Desc = &mp4io.AVC1Desc{
			...
			Depth:                24,
			ColorTableId:         -1,
			Conf:                 &mp4io.AVC1Conf{Data: codec.ConfigBytes},
		}
	...
	case *aac.Codec:
		self.sample.SampleDesc.MP4ADesc = &mp4io.MP4ADesc{
			DataRefIdx:       1,
			NumberOfChannels: int16(codec.Config.ChannelLayout.Count()),
	...
	default:
		err = fmt.Errorf("mp4: codec is not supported type=%+v ", reflect.TypeOf(codec))
	}
...
func (self *Muxer) WritePacket(pkt av.Packet) (err error) {
	...
	stream, ok := self.streams[pkt.Type]
	if !ok {
		streams, err := self.w.Streams()
		if err != nil {
			return err
		}
		if err := self.addStreams(streams); err != nil {
			return err
		}
	}
	...
	stream, ok = self.streams[pkt.Type]
	if !ok {
		return errors.Errorf("Unsuported stream for packet type:%v", pkt.Type)
	}
...
func (self *Stream) writePacket(pkt av.Packet, rawdur time.Duration) (err error) {
	if rawdur < 0 {
		err = fmt.Errorf("mp4: stream#%v time=%v < lasttime=%v", pkt.Type, pkt.Time, self.lastpkt.Time)
		return
	}
	...
	if self.sample.CompositionOffset != nil {
		offset := uint32(self.timeToTs(pkt.CTime))
